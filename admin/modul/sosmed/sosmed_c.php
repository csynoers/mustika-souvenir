...
if(!isset($_SESSION['user_session'])){
  header("Location: index.php");
	...
	if ($_POST['act']=='btn-edit') {
		if(isset($_POST["id"])){
			$output = array();
			$statement = $database->select($fields="*", $table="sosmed", $where_clause="WHERE id_sosmed = $_POST[id]", $fetch="");
				$output["title"] = $statement["title"];
				$output["link"] = $statement["link"];
				$output["status"] = $statement["status"];
				if($statement["image"] != '')
				{
					$output['fupload'] = '<img src="../joimg/sosmed/'.$statement["image"].'" class="img-thumbnail" width="100px" height="auto" /><input type="hidden" name="hidden_fupload" value="'.$statement["image"].'" />';
				}
				else
				{
	...
	if(isset($_POST["operation"]))
	{
		...
		if($_POST["operation"] == "Edit")
		{
			...
			if(!empty($lokasi_file))
			{
	            ...
	            $show   = $database->select($fields="image", $table="sosmed", $where_clause="WHERE id_sosmed = '$_POST[id]'", $fetch='');
				if($show['image'] != '')
				{
					unlink("../../../joimg/sosmed/$show[image]");
				}
	...
	if ($_POST['act']=='btn-delete') {
		if(isset($_POST["id"]))
		{
			...
			{
				unlink("../../../joimg/sosmed/$show[image]");
	            $database->delete($table="sosmed", $fields_key="id_sosmed", $id="$_POST[id]");
			}
			else
			{
	            $database->delete($table="sosmed", $fields_key="id_sosmed", $id="$_POST[id]");
