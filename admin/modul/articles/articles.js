$(document).ready(function(){
  $('#add_button').click(function(){
    $('#form_input')[0].reset();
    $('.modal-title').text("Add New articles");
    $('#action').val("Add");
    $('#operation').val("Add");
    $('#uploaded_image').html('');
  });
  
  var dataTable = $('#articles_data').DataTable({
    "processing":true,
    "serverSide":true,
    "order":[],
    "ajax":{
      url:"modul/articles/load_data.php",
      type:"POST"
    },
    "columnDefs":[
      {
  ...
  $(document).on('submit', '#form_input', function(event){
    ...
    if(title != '' && description != ''){
      $.ajax({
        url:"modul/articles/articles_c.php",
        method:'POST',
  ...
  $(document).on('click', '.update', function(){
    ...
    $.ajax({
      ...
      success:function(data)
      {
        ...
        $('.modal-title').text("Edit articles");
        $('#id').val(id);
